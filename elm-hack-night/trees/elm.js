...
Elm.Main.make = function (_elm) {
   ...
   $TreeView = Elm.TreeView.make(_elm);
   var myTree = $Tree.insert(434)($Tree.insert(3)($Tree.insert(2)($Tree.insert(41)($Tree.insert(42)($Tree.empty)))));
   var main = A2($TreeView.draw,
   800,
   myTree);
...
Elm.Tree.make = function (_elm) {
   ...
   var empty = Empty;
   var singleton = function (el) {
      return A3(Node,
      el,
      Empty,
      Empty);
   };
   var insert = F2(function (el,
   tree) {
      return function () {
         switch (tree.ctor)
         {case "Empty":
            return singleton(el);
            case "Node": return _U.cmp(el,
              tree._0) < 0 ? A3(Node,
              tree._0,
              A2(insert,el,tree._1),
              tree._2) : _U.cmp(el,
              tree._0) > 0 ? A3(Node,
              tree._0,
              tree._1,
              A2(insert,el,tree._2)) : tree;}
         _U.badCase($moduleName,
         "between lines 20 and 76");
      }();
   });
...
Elm.TreeView.make = function (_elm) {
   "use strict";
   _elm.TreeView = _elm.TreeView || {};
   if (_elm.TreeView.values)
   return _elm.TreeView.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $Text = Elm.Text.make(_elm),
   $Tree = Elm.Tree.make(_elm);
   var treeToForm = function (tree) {
      return function () {
         switch (tree.ctor)
         {case "Empty":
            return {ctor: "_Tuple2"
                   ,_0: A2($Graphics$Collage.filled,
                   $Color.orange,
            ...
            case "Node":
            return function () {
                 var $ = treeToForm(tree._2),
                 rightForm = $._0,
                 rightDepth = $._1;
                 var rightPoint = {ctor: "_Tuple2"
                                  ,_0: 30 * Math.pow(2,rightDepth)
                                  ,_1: -30};
                 var $ = treeToForm(tree._1),
                 leftForm = $._0,
                 leftDepth = $._1;
                 var leftPoint = {ctor: "_Tuple2"
                                 ,_0: -30 * Math.pow(2,leftDepth)
                                 ,_1: -30};
                 var treeForm = $Graphics$Collage.group(_L.fromArray([$Graphics$Collage.traced($Graphics$Collage.solid($Color.black))(A2($Graphics$Collage.segment,
                                                                     {ctor: "_Tuple2"
                                                                     ,_0: 0
                                                                     ,_1: 0},
                                                                     leftPoint))
                                                                     ...
                                                                     ,_1: 0},
                                                                     rightPoint))
                                                                     ,$Graphics$Collage.filled($Color.blue)(A2($Graphics$Collage.oval,
                                                                     20,
                                                                     20))
                                                                     ,$Graphics$Collage.text($Text.color($Color.white)($Text.fromString($Basics.toString(tree._0))))
         ...
         _U.badCase($moduleName,
         "between lines 41 and 71");
      }();
   };
   var draw = F2(function (viewWidth,
   tree) {
      return function () {
         var $ = treeToForm(tree),
         treeForm = $._0,
         depth = $._1;
         var treeWidth = 30 * Math.pow(2,
         depth) * 2;
         var scaleFactor = _U.cmp(treeWidth,
         viewWidth - 20) > 0 ? viewWidth / treeWidth : 1;
         var treeHeight = 30 * depth * scaleFactor;
         return $Html.fromElement(A3($Graphics$Collage.collage,
         $Basics.round(viewWidth),
         $Basics.round(treeHeight) + 40,
         _L.fromArray([$Graphics$Collage.move({ctor: "_Tuple2"
                                              ,_0: 0
                                              ,_1: treeHeight / 2})($Graphics$Collage.scale(scaleFactor)(treeForm))])));
      }();
   });
   _elm.TreeView.values = {_op: _op
                          ,draw: draw
                          ,treeToForm: treeToForm};
   return _elm.TreeView.values;
};
