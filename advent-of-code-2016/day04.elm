...
parse : String -> Result String RoomData
parse string =
    let
        matches =
            find (AtMost 1) roomDataRegex string
        ...
        match =
            case matches of
                ...
                _ ->
                    Err "input doesn't match roomDataRegex"

        extractFields match =
            case match.submatches of
                [ Just encryptedName, Just sectorId, Just checksum ] ->
                    Ok { encryptedName = encryptedName, sectorId = sectorId, checksum = toList checksum }

                _ ->
                    Err "failed to capture all submatches"

        parseSectorId parsedMatch =
            toInt parsedMatch.sectorId
                |> Result.map (\sectorId -> { parsedMatch | sectorId = sectorId })
    in
        match
            |> Result.andThen extractFields
            |> Result.andThen parseSectorId


isRoomValid : RoomData -> Bool
isRoomValid roomData =
    (log "computed " (checksum roomData.encryptedName)) == (log "provided" roomData.checksum)
