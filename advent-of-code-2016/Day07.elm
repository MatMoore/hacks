...
instructions =
    lines """xdsqxnovprgovwzkus[fmadbfsbqwzzrzrgdg]aeqornszgvbizdm
...
parse : Int -> Char -> Parser -> Parser
parse windowSize next parser =
    case ( next, parser ) of
        ( ']', HyperNet _ ) ->
            SuperNet []
        ...
        ( new, HyperNet chars ) ->
            (new :: chars) |> take windowSize |> HyperNet

        ( new, SuperNet chars ) ->
            (new :: chars) |> take windowSize |> SuperNet


supportsTLS : String -> Bool
supportsTLS instruction =
    let
        chars : List Char
        chars =
            String.toList instruction
        ...
        substrings =
            List.scanl (parse 4) (SuperNet []) chars
    in
        (List.any abba substrings) && (not (List.any hypernetAbba substrings))


supportsSSL : String -> Bool
supportsSSL instruction =
    let
        chars : List Char
        chars =
            String.toList instruction
        ...
        substrings =
            List.scanl (parse 3) (SuperNet []) chars

        babs =
            List.filter bab substrings

        abaForBab : Parser -> Bool
        abaForBab bab =
            case bab of
                HyperNet (b :: a :: _) ->
                    List.any (\aba -> aba == SuperNet [ a, b, a ]) substrings

                _ ->
                    False
    in
        List.any abaForBab babs


checkTLS instruction state =
    if supportsTLS instruction then
        { state | tls = instruction :: state.tls }
    else
        { state | not_tls = instruction :: state.not_tls }


checkSSL instruction state =
    if supportsSSL instruction then
        { state | tls = instruction :: state.tls }
    else
        { state | not_tls = instruction :: state.not_tls }
...
view model =
    div []
        [ p [] [ "Answer: " ++ (model.state.tls |> List.length |> toString) |> text ]
        , p [] [ "Remaining IPs: " ++ (model.instructions |> List.length |> toString) |> text ]
        ]
