...
import Hacking exposing (stepByStepProgram)
...
type alias Bot =
    { lowest : Maybe Microchip
    , other : Maybe Microchip
    , errors : List String
    , instructions : List GiveInstruction
    }
...
type Instruction
    = BotTransfer Int GiveInstruction
    | Input Int Int
            ...
            case match.submatches of
                [ Just fromBot, lowTarget, lowNumber, highTarget, highNumber ] ->
                    ...
                    in
                        toInt fromBot
                            |> Result.map
                                (\botNumber -> BotTransfer botNumber { high = highTransfer, low = lowTransfer })
...
parsedInstructions =
    String.lines input |> List.filterMap (Result.toMaybe << parseInstruction)


_ =
    log "input" (List.length (String.lines input))
_ =
    log "parsed" (List.length parsedInstructions)


instructBot : GiveInstruction -> Bot -> Bot
instructBot instruction bot =
    { bot | instructions = bot.instructions ++ [ instruction ] }


giveToBot : Microchip -> Bot -> Bot
giveToBot chip bot =
    case ( bot.lowest, bot.other ) of
        ( Just a, Nothing ) ->
            if chip >= a then
                { bot | other = Just chip, lowest = Just a }
            else
                { bot | lowest = Just chip, other = Just a }

        ( Nothing, Nothing ) ->
            { bot | lowest = Just chip }

        ( Just a, Just b ) ->
            { bot | errors = "Already holding two chips" :: bot.errors }
...
newBot : Bot
newBot =
    { lowest = Nothing, other = Nothing, errors = [], instructions = [] }


cloneChip maybeChip target initialModel =
    case ( maybeChip, target ) of
        ( Just chip, Just (GiveToBot highBotNum) ) ->
            let
                highBot =
                    Dict.get highBotNum initialModel.bots |> Maybe.withDefault newBot
            in
                { initialModel | bots = Dict.insert highBotNum (giveToBot chip highBot) initialModel.bots }

        ( Just chip, Just (PutInBin binNum) ) ->
            let
                bin =
                    Dict.get binNum initialModel.bins |> Maybe.withDefault []
            in
                { initialModel | bins = Dict.insert binNum (chip :: bin) initialModel.bins }

        _ ->
            { initialModel | bots = Dict.empty }


resetBot : Int -> Model -> Model
resetBot botNum model =
    let
        bot =
            Dict.get botNum model.bots |> Maybe.withDefault newBot

        updatedBot =
            { bot | lowest = Nothing, other = Nothing }
    in
        { model | bots = Dict.insert botNum updatedBot model.bots }


runGiveInstruction : Int -> Bot -> GiveInstruction -> Model -> Model
runGiveInstruction botNum initialBot instruction initialModel =
    cloneChip initialBot.other instruction.high initialModel
        |> cloneChip initialBot.lowest instruction.low
        |> resetBot botNum
...
runBots model =
    let
        runBot : Int -> Bot -> Model -> Model
        runBot botNum bot model =
            case ( bot.lowest, bot.other, bot.instructions ) of
                ( Just l, Just o, first :: rest ) ->
                    let
                        updatedBot =
                            if l == 17 && o == 61 then
                                { bot | instructions = rest, errors = "I COMPARED THE CHIPS" :: bot.errors }
                            else
                                { bot | instructions = rest }

                        newModel =
                            { model | bots = Dict.insert botNum updatedBot model.bots }

                        updated =
                            runGiveInstruction botNum updatedBot first newModel
                    in
                        updated
        ...
        updatedModel =
            Dict.foldl runBot model model.bots
    in
        if updatedModel == model then
            model
        else
            runBots updatedModel
...
runInstruction instruction model =
    let
        newBots =
            case instruction of
                BotTransfer bot gifts ->
                    Dict.insert bot (instructBot gifts (Dict.get bot model.bots |> Maybe.withDefault newBot)) model.bots

                Input bot value ->
                    Dict.insert bot (giveToBot value (Dict.get bot model.bots |> Maybe.withDefault newBot)) model.bots
...
view { state, instructions } =
    let
        botItems =
            List.map
                (\bot -> li [] [ bot |> toString |> text ])
                (Dict.toList state.bots)

        binItems =
            List.map
                (\bin -> li [] [ bin |> toString |> text ])
                (Dict.toList state.bins)
    in
        div []
            [ (h1 [] [ text "Bots" ])
            , (ul [] botItems)
